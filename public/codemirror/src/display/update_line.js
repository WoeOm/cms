...
function updateLineClasses(cm, lineView) {
  ...
  const textClass = lineView.textClass
...
function updateLineGutter(cm, lineView, lineN, dims) {
  ...
  if (lineView.line.gutterClass) {
    ...
    lineView.gutterBackground = elt(
      ...
      `left: ${cm.options.fixedGutter
        ? dims.fixedPos
        : -dims.gutterTotalWidth}px; width: ${dims.gutterTotalWidth}px`,
    );
  ...
  if (cm.options.lineNumbers || markers) {
    const wrap = ensureLineWrapped(lineView);
    const gutterWrap = (lineView.gutter = elt(
      'div',
      null,
    ...
    if (
      ...
      lineView.lineNumber = gutterWrap.appendChild(
        elt(
          ...
          `left: ${dims.gutterLeft['CodeMirror-linenumbers']}px; width: ${cm
        ...
        if (found)
          gutterWrap.appendChild(
            elt(
              ...
              `left: ${dims.gutterLeft[id]}px; width: ${dims.gutterWidth[
                id
              ]}px`,
            ),
          );
