...
class NativeScrollbars {
  constructor(place, scroll, cm) {
    this.cm = cm;
    const vert = (this.vert = elt(
      'div',
      [elt('div', null, null, 'min-width: 1px')],
      'CodeMirror-vscrollbar',
    ));
    const horiz = (this.horiz = elt(
      'div',
  ...
  update(measure) {
    ...
    if (needsV) {
      ...
      this.vert.firstChild.style.height = `${Math.max(
        0,
    ...
    if (needsH) {
      ...
      this.horiz.firstChild.style.width = `${Math.max(
        0,
