...
export function drawSelectionCursor(cm, head, output) {
  ...
  if (pos.other) {
    ...
    const otherCursor = output.appendChild(
...
function drawSelectionRange(cm, range, output) {
  ...
  function drawForLine(line, fromArg, toArg) {
    ...
    iterateBidiSections(
      ...
      (from, to, dir, i) => {
        const fromPos = coords(from, dir == 'ltr' ? 'left' : 'right');
        const toPos = coords(to - 1, dir == 'ltr' ? 'right' : 'left');
        if (dir == 'ltr') {
          const fromLeft =
            fromArg == null && from == 0 ? leftSide : fromPos.left;
          const toRight =
            toArg == null && to == lineLen ? rightSide : toPos.right;
          ...
          const toLeft = toArg == null && to == lineLen ? leftSide : toPos.left;
          if (toPos.top - fromPos.top <= 3) {
            // Single line
            ...
            if (i) {
              const topEnd = wrappedLineExtentChar(cm, lineObj, null, from).end;
            ...
            if (i < order.length - 1 || true) {
              const botStart = wrappedLineExtentChar(cm, lineObj, null, to)
                .begin;
  ...
  if (sFrom.line == sTo.line) {
    ...
    const leftEnd = drawForLine(
      sFrom.line,
      sFrom.ch,
      singleVLine ? fromLine.text.length + 1 : null,
    ).end;
    const rightStart = drawForLine(sTo.line, singleVLine ? 0 : null, sTo.ch)
      .start;
    if (singleVLine) {
