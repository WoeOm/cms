...
export function moveVisually(cm, line, start, dir) {
  ...
  let prep;
  const getWrappedLineExtent = ch => {
    if (!cm.options.lineWrapping) return { begin: 0, end: line.text.length };
    prep = prep || prepareMeasureForLine(cm, line);
    return wrappedLineExtentChar(cm, line, prep, ch);
  };
  const wrappedLineExtent = getWrappedLineExtent(
  ...
  const searchInVisualLine = (partPos, dir, wrappedLineExtent) => {
    const getRes = (ch, moveInStorageOrder) =>
      moveInStorageOrder
