...
function updateExternalMeasurement(cm, line) {
  ...
  const view = (cm.display.externalMeasured = new LineView(
    cm.doc,
    line,
    lineN,
  ));
...
export function wrappedLineExtentChar(cm, lineObj, preparedMeasure, target) {
  if (!preparedMeasure) preparedMeasure = prepareMeasureForLine(cm, lineObj);
  const targetTop = intoCoordSystem(
    cm,
    lineObj,
...
function coordsCharInner(cm, lineObj, lineNo, x, y) {
  ...
  if (order) {
    const part = (cm.options.lineWrapping
...
function coordsBidiPartWrapped(
  ...
  const { begin, end } = wrappedLineExtent(cm, lineObj, preparedMeasure, y);
  let part = null,
    closestDist = null;
  for (let i = 0; i < order.length; i++) {
    ...
    const ltr = p.level != 1;
    const endX = measureCharPrepared(
      cm,
      preparedMeasure,
