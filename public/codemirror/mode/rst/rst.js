...
(function(mod) {
  ...
  CodeMirror.defineMode(
    ...
    (config, options) => {
      ...
      const mode = CodeMirror.getMode(config, options.backdrop || 'rst-base');
    ...
    config => {
      ...
      function format(string) {
        ...
        return string.replace(
          /{(\d+)}/g,
          (match, n) => (typeof args[n] !== 'undefined' ? args[n] : match),
        );
      }
      ...
      const rx_explicit = new RegExp(format('^\\.\\.{0}', SEPA));
      const rx_link = new RegExp(format('^_{0}:{1}|^__:{1}', REF_NAME, TAIL));
      const rx_directive = new RegExp(format('^{0}::{1}', REF_NAME, TAIL));
      ...
      const rx_citation = new RegExp(format('^\\[{0}\\]{1}', REF_NAME, TAIL));

      const rx_substitution_ref = new RegExp(format('^\\|{0}\\|', TEXT1));
      const rx_footnote_ref = new RegExp(
        format('^\\[(?:\\d+|#{0}?|\\*)]_', REF_NAME),
      );
      const rx_citation_ref = new RegExp(format('^\\[{0}\\]_', REF_NAME));
      const rx_link_ref1 = new RegExp(format('^{0}__?', REF_NAME));
      const rx_link_ref2 = new RegExp(format('^`{0}`_', TEXT2));
      ...
      const rx_role = new RegExp(format('^:{0}:{1}', NAME, TAIL));
      ...
      function to_normal(stream, state) {
        ...
        } else if (stream.next()) change(state, to_normal);
