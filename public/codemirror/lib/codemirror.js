...
let Delayed = function() {this.id = null;};
  ...
  // handling this'.
let Pass = {toString(){return "CodeMirror.Pass"}};

  // Reused option objects for setSelection & friends
let sel_dontScroll = {scroll: false};
let sel_mouse = {origin: "*mouse"};
let sel_move = {origin: "+move"};
...
let hasSelection = window.getSelection ? function (te) {
        try {
...
let hasCopyEvent = (function () {
  ...
  let builder = {pre: eltP("pre", [content], "CodeMirror-line"), content,
      col: 0,
      pos: 0,
      cm,
      trailingSpace: false,
    ...
    let line = i ? lineView.rest[i - 1] : lineView.line, order = (void 0);
      builder.pos = 0;
  ...
  function insertLineContent(line, builder, styles) {
    ...
    for (;;) {
      if (nextChange == pos) {
        ...
        collapsed = null;
        nextChange = Infinity;
      let foundBookmarks = [], endStyles = (void 0);
...
let nullRect = {left: 0, right: 0, top: 0, bottom: 0};
  ...
  let begin = findFirst((ch) => { return measureCharPrepared(cm, preparedMeasure, ch - 1).bottom <= y; }, end, 0);
    end = findFirst(
      function(ch) {
  ...
  function boxIsAfter(box, x, y, left) {
    return box.bottom <= y
      ? false
      : box.top > y ? true : (left ? box.left : box.right) > x;
  }
    ...
    function drawForLine(line, fromArg, toArg) {
      ...
      iterateBidiSections(
        ...
        function(from, to, dir, i) {
          ...
          if (dir == 'ltr') {
            var fromLeft =
              fromArg == null && from == 0 ? leftSide : fromPos.left;
        let toRight = toArg == null && to == lineLen ? rightSide : toPos.right;
            ...
            // RTL
            var fromRight =
              fromArg == null && from == 0 ? rightSide : fromPos.right;
            ...
            if (toPos.top - fromPos.top <= 3) {
              ...
              if (i) {
                ...
                topLeft = coords(
                  topEnd - (/\s/.test(lineObj.text.charAt(topEnd - 1)) ? 2 : 1),
                  'left',
                ).left;
              }
              ...
              if (i < order.length - 1 || true) {
                var botStart = wrappedLineExtentChar(cm, lineObj, null, to)
                  .begin;
  ...
  function updateHeightsInViewport(cm) {
    ...
    let cur = display.view[i], height = (void 0);
      if (cur.hidden) {
        continue;
      }
      if (ie && ie_version < 8) {
...
let scrollbarModel = {"native": NativeScrollbars, "null": NullScrollbars};
  ...
  Selection.prototype.equals = function(other) {
    ...
    let here = this$1.ranges[i], there = other.ranges[i];
      if (
  ...
  BranchChunk.prototype = {
    ...
    removeInner: function removeInner(at, n) {
      ...
      let child = this$1.children[i], sz = child.chunkSize();
        if (at < sz) {
  ...
  TextMarker.prototype.clear = function() {
    ...
    for (let i = 0; i < this.lines.length; ++i) {
      var line = this$1.lines[i];
      let span = getMarkedSpanFor(line.markedSpans, this$1);
    ...
    let visual = visualLine(this$1.lines[i$1]), len = lineLength(visual);
  ...
  TextMarker.prototype.detachLine = function(line) {
    ...
    if (!this.lines.length && this.doc.cm) {
    let op = this.doc.cm.curOp;(op.maybeHiddenMarkers || (op.maybeHiddenMarkers = [])).push(this);
    }
  };
  ...
  let wrappedLineExtent = getWrappedLineExtent(start.sticky == "before" ? mv(start, -1) : start.ch);
    ...
    let getRes = function (ch, moveInStorageOrder) { return moveInStorageOrder
...
let stopSeq = new Delayed;
  ...
  function configureMouse(cm, repeat, event) {
    ...
    if (value.unit == null) {
      ...
      value.unit = rect
        ? 'rectangle'
        : repeat == 'single' ? 'char' : repeat == 'double' ? 'word' : 'line';
    }
...
let Init = {toString(){return "CodeMirror.Init"}};
  ...
  function applyTextInput(cm, inserted, deleted, sel, origin) {
    ...
    let changeEvent = {from, to, text: multiPaste ? multiPaste[i$1 % multiPaste.length] : textLines,
        origin:
  ...
  let helpers = CodeMirror.helpers = {};
    ...
    CodeMirror.prototype = {
      ...
      indentSelection: methodOp(function(how) {
        ...
        for (let i = 0; i < ranges.length; i++) {
          ...
          let from = range$$1.from(), to = range$$1.to();
            ...
            let newRanges = this$1.doc.sel.ranges;
            if (
              from.ch == 0 &&
      ...
      let dir = 1, x = goalColumn;
        ...
        let coords = cursorCoords(this$1, cur, "div");
          if (x == null) {
            x = coords.left;
          } else {
            coords.left = x;
